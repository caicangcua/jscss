...
ComApp.Store.Order = function (params) {
    var __$cartPopup = function (__$shared, cartFuck,idx) {
        ...
        , renderEditCart = function (contentElement,oInfo) {
            ...
            //
            var _$t = timeDeli, defTime = _$t[0]; deliverytime.empty();
            ...
            $(dathang[1]).fadeIn('slow').find('#fastcartform input').change(function () {
                ...
                var _$ip = $(txt);
                if (oInfo.hasOwnProperty(_$ip.attr('name'))) {
                    _$ip.val(oInfo[_$ip.attr('name')]);
                    _$$cbCart[_$ip.attr('name')] = oInfo[_$ip.attr('name')];
                }
            });
        ...
        , renderPopupCart = function (contentElement) {
            ...
            var fuckModel = function () {
                ...
                this.hoan_tat = hoan_$tat; this.csshoan_tat = csshoan_$tat;
            }
            , dynamicEL = function () {
                switch (parseInt(dulieuR['statux'])) {
                    ...
                    case 1: {// approved , wait shpping
                        cssdat_$hang('done'); csscho_$duyet('done'); cssgiao_$hang('active');
                        cho_$duyet(gbM('S2_0074'));
                        break;
                    }
                    case 2: {// shipping
                        cssdat_$hang('done'); csscho_$duyet('done'); cssgiao_$hang('done'), csshoan_$tat('active');
                        cho_$duyet(gbM('S2_0074'));
                        break;
                    }
                    case 10: {
            ...
            , hoan_$tat = ko.observable(gbM('S2_0073')), csshoan_$tat = ko.observable('')
    ...
    , renderOrderList = function (rst) {
        ...
        for (var i = 0; i < newO.length; i++) {
            ...
            newDIV.find('.orderscol-price>div').html(_Tien(newO[i].tien, 0));
    ...
    , viewModel = {
        ...
        , showitemdetail: function (e, arg) {
            scrLocked();
            var that = this, itemR = $(arg.target).closest('.ordersrow').attr('id');
            setTimeout(function () {
                if (!$cartTMP) {
                    $.get(gitpath + "template/tmpcart.html", function (data) {
                        $cartTMP = $(data); __$cartPopup(__$shared, $cartTMP.clone(), itemR);
                    });
                } else {
